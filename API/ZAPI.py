...
TPL_MARKET = 'http://api.bitkk.com/data/v1/ticker?market={MARKET}'
...
class ZApi:
	...
	def get_orders(self, market, page, type):
		...
		:return: result
		"""
		params = 'accesskey=%s&currency=%s&method=getOrders&pageIndex=%s&tradeType=%s' % (
	...
	def get_orders_new(self, market, page_index, page_size, type):
		...
		:return: result
		"""
		params = 'accesskey=%s&currency=%s&method=getOrdersNew&pageIndex=%s&pageSize=%s&tradeType=%s' % (
	...
	def get_orders_ignore_tader_type(self, market, page_index, page_size):
		...
		:return: result
		"""
		params = 'accesskey=%s&currency=%s&method=getOrdersIgnoreTradeType&pageIndex=%s&pageSize=%s' % (
	...
	def get_unfinished_orders_ignore_trade_type(self, market, page_index, page_size):
		...
		:return: result
		"""
		params = 'accesskey=%s&currency=%s&method=getUnfinishedOrdersIgnoreTradeType&pageIndex=%s&pageSize=%s' % (
		self._access_key_, market, page_index, page_size)
		# print(params)
	...
	def zhuanzhang_zi(self, amount, currency, fromUerName, toUserName):
		...
		:return: result
		"""
		params = 'accesskey=%s&amount=%s&currency=%s&fromUserName=%s&method=doTransferFunds&toUserName=%s' % (
	...
	def get_withdraw_record(self, currency, page_index, page_size):
		...
		:return:
		"""
		params = 'accesskey=%s&currency=%s&method=getWithdrawRecord&pageIndex=%s&pageSize=%s' % (
	...
	def get_charge_record(self, currency, page_index, page_size):
		...
		:return: result
		"""
		params = 'accesskey=%s&currency=%s&method=getChargeRecord&pageIndex=%s&pageSize=%s' % (
		self._access_key_, currency, page_index, page_size)
	...
	def withdraw(self, currency, amount, fees, itransfer, addr, pwd):
		...
		:return: result
		"""
		params = 'accesskey=%s&amount=%s&currency=%s&fees=%s&itransfer=%s&method=withdraw&receiveAddr=%s&safePwd=%s' % (
		self._access_key, amount, currency, fees, itransfer, addr, pwd)
